...
public class Tablero {
    ...
    public ArrayList<ArrayList<int[]>> getMovsPosibles() {
    	...
    	if (filas == 0) {
    	    // La lista est� vac�a, no hay nada que hacer
    	    return movsPosibles ; // de momento est� vac�a
    	}
...
/*
   ...
   */
    ...
    public Tablero comprimirTablero() {
    	...
    	if (filas == 0) {
    	    // La lista est� vac�a, no hay nada que hacer
    	    return this; // O devolver una nueva instancia de Tablero vac�a, seg�n tus necesidades
    	}
    	...
    	for (int i = 0; i < filas; i++) {
    	    if (this.getTablero().get(i).isEmpty()) {
    	        // La fila est� vac�a, no hay nada que hacer
    	        continue;
    	    }
    	    ...
    	    for (int j = 0; j < columnas; j++) {
    	        ...
    	        int k = i;
    	        // Verificar si la lista en la posici�n k tiene elementos antes de acceder a ellos
    ...
    public void imprimirTablero() {
        for (int i = 0; i < filas; i++) {
            for (int j = 0; j < columnas; j++) {
                System.out.print(this.tablero.get(i).get(j).getColor()); // Puedes ajustar la forma de impresión según tus necesidades
            }
